...
def create_driver():
    options = Options()
    driver = webdriver.Chrome(options=options) # 크롬드라이버 자동 설정 되므로 비추천.
    return driver
    ...
    # 크롬 업데이트와 크롬드라이버 업데이트가 상이하므로, 특정 버전대로 고정하여 사용해야 오류가 덜남. ex. 114 추천
    # https://lyaaaan.medium.com/%EB%A7%A5-%EC%85%80%EB%A0%88%EB%8A%84-%ED%95%B4%EA%B2%B0-%EB%B0%A9%EB%B2%95-181fa335f207
...
# CSV 파일 로드
data = pd.read_csv('./merge/modified_red_image_1.csv')
...
data.to_csv('./wine/red_updated_vivino_data1.csv', index=False)
